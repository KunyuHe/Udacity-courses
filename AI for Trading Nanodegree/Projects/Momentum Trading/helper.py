...
def download_quandl_dataset(quandl_api_key, database, dataset, save_path,
                            columns, tickers, start_date, end_date):
    """
    ...
    bulk_download_url = \
    scrape_request.json()['datatable_bulk_download']['file']['link']

    with tempfile.TemporaryDirectory() as tmp_dir:
        tmp_wiki_file = tmp_dir + 'tmp.zip'

        bulk_download_request = requests.get(bulk_download_url, stream=True,
                                             cookies=scrape_request.cookies)
        total_size = int(
            bulk_download_request.headers.get('content-length', 0));
        ...
        with tqdm(total=5, desc='Transforming Data', unit='Action') as pbar:
            ...
            tmp_df = tmp_df[tmp_df['date'].isin(
                pd.date_range(start_date, end_date))]  # Filter unused dates
            tmp_df = tmp_df[
                tmp_df['ticker'].isin(tickers)]  # Filter unused tickers
            pbar.update(1)
...
def generate_config():
    return {'showLink': False, 'displayModeBar': False,
