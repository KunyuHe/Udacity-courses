...
def _generate_buy_annotations(prices, signal):
    return [{
        'x': index, 'y': price, 'text': 'Long',
        'bgcolor': helper.color_scheme['background_label'],
        'ayref': 'y', 'ax': 0, 'ay': 20}
...
def _generate_second_tetration_stock(stock_symbol, dates):
    ...
    sector_stock['base_line'] = sector_stock['base_line'] + all_noise[0] * \
                                sector_stock['base_line']
    sector_stock['adj_open'] = sector_stock['base_line'] + all_noise[1] * \
                               sector_stock['base_line']
    sector_stock['adj_close'] = sector_stock['base_line'] + all_noise[2] * \
                                sector_stock['base_line']
    sector_stock['adj_high'] = sector_stock['base_line'] + all_noise[3] * \
                               sector_stock['base_line']
    sector_stock['adj_low'] = sector_stock['base_line'] + all_noise[4] * \
                              sector_stock['base_line']
...
def generate_tb_sector(dates):
    ...
    symbol_length = 6
    stock_names = [
        'kaufmanniana', 'clusiana', 'greigii', 'sylvestris', 'turkestanica',
        'linifolia', 'gesneriana',
        'humilis', 'tarda', 'saxatilis', 'dasystemon', 'orphanidea',
        'kolpakowskiana', 'praestans',
        'sprengeri', 'bakeri', 'pulchella', 'biflora', 'schrenkii', 'armena',
        'vvedenskyi', 'agenensis',
        'altaica', 'urumiensis']

    return [
        _generate_second_tetration_stock(stock_name[:symbol_length].upper(),
                                         dates)
...
def plot_high_low(prices, lookback_high, lookback_low, title):
    ...
    offline_py.iplot(
        {'data': [stock_trace, high_trace, low_trace], 'layout': layout},
        config=config)
...
def plot_lookahead_prices(prices, lookahead_price_list, title):
    ...
    colors = Color(helper.color_scheme['low_value']) \
        .range_to(Color(helper.color_scheme['high_value']),
                  len(lookahead_price_list))

    traces = [_generate_stock_trace(prices)]
    for (lookahead_prices, lookahead_days), color in zip(lookahead_price_list,
                                                         colors):
        traces.append(
            go.Scatter(
...
def plot_signal_returns(prices, signal_return_list, titles):
    ...
    stock_trace = _generate_stock_trace(prices)
    for (signal_return, signal, lookahead_days), color, title in zip(
            signal_return_list, colors, titles):
        ...
        offline_py.iplot(
            {'data': [stock_trace, signal_return_trace], 'layout': layout},
            config=config)
...
def plot_signal_histograms(signal_list, title, subplot_titles):
    ...
    fig = py.tools.make_subplots(rows=1, cols=len(signal_series_list),
                                 subplot_titles=subplot_titles,
                                 print_grid=False)
    fig['layout'].update(title=title, showlegend=False)

    for series_i, (signal_series, color) in enumerate(
            zip(signal_series_list, colors), 1):
