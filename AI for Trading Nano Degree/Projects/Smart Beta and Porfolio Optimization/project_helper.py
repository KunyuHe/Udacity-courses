...
def _generate_hover_text(x_text, y_text, z_values, x_label, y_label, z_label):
    ...
    hover_text = x_label + ':  ' + padding_len[
        0] * ' ' + x_hover_text_values + '<br>' + \
                 y_label + ':  ' + padding_len[
                     1] * ' ' + y_hover_text_values.T + '<br>' + \
                 z_label + ': ' + padding_len[2] * ' ' + float_to_str(z_values)

    return hover_text


def _generate_heatmap_trace(df, x_label, y_label, z_label, scale_min,
                            scale_max):
    hover_text = _generate_hover_text(df.index, df.columns, df.values.T,
...
def large_dollar_volume_stocks(df, price_column, volume_column, top_percent):
    ...
    return dollar_traded.sort_values().tail(
        int(len(dollar_traded) * top_percent)).index.values.tolist()
...
def plot_weights(weights, title):
    ...
    trace = _generate_heatmap_trace(weights.sort_index(axis=1, ascending=False),
                                    'Date', 'Ticker', 'Weight', 0.0, 0.2)
    layout = go.Layout(
        title=title,
        xaxis={'title': 'Dates'},
        yaxis={'title': 'Tickers'})
...
def plot_returns(returns, title):
    config = helper.generate_config()
    graph_path = 'graphs/{}.html'.format(_sanatize_string(title))
    trace = _generate_heatmap_trace(returns.sort_index(axis=1, ascending=False),
                                    'Date', 'Ticker', 'Weight', -0.3, 0.3)
    layout = go.Layout(
        title=title,
        xaxis={'title': 'Dates'},
        yaxis={'title': 'Tickers'})
