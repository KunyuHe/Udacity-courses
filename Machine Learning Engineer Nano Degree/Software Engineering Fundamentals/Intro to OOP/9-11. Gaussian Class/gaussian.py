...
class Gaussian():
    ...
    def calculate_stdev(self, sample=True):
        ...
        if sample:
            self.stdev = math.sqrt(total / float(len(self.data) - 1))
        else:
    ...
    def plot_histogram(self):
        ...
        """
        plt.hist(self.data)
        plt.title("Histogram of the Data", size=18)
    ...
    def pdf(self, x):
        ...
        """
        return 1 / (self.stdev * math.sqrt(2 * math.pi)) * math.exp(
            -1 / 2 * ((x - self.mean) / self.stdev) ** 2)
